...
import plotly.express as px     # interactive charts (bar, pie, scatter, etc.)
import pandas as pd             # data tables
import matplotlib.pyplot as plt # used for the word cloud rendering
from wordcloud import WordCloud  # generates the word cloud image
...
st.set_page_config(
    page_title="🔥 Trending Now",   # browser tab title
...
# We use Google Fonts (Syne + DM Sans) for a modern, editorial feel.
...
# Refresh button — clears Streamlit's data cache so fresh data is fetched
col_btn, col_time, _ = st.columns([1, 2, 5])
with col_btn:
    if st.button("🔄 Refresh"):
        st.cache_data.clear()       # clears ALL cached functions below
        st.rerun()                  # re-runs the entire script

#  DATA LOADING  (cached for 10 minutes)

@st.cache_data(ttl=600)    # ttl=600 seconds = 10 minutes; avoids hammering APIs
def load_all_data():
    """
    Fetches and enriches data from all three sources.
    Returns three DataFrames: reddit, hackernews, github.
    """
    reddit = add_sentiment(fetch_reddit())          # fetch + label sentiments
    hn     = add_sentiment(fetch_hackernews())      # fetch + label sentiments
    github = fetch_github_trending()                # no sentiment needed here
    return reddit, hn, github

# Show a spinner while the first fetch runs (cached after that)
...
with tab3:
    ...
    if github_df.empty:
        ...
        fig3 = px.scatter(
            ...
            x="stars",
            y="forks",
            size="stars",              # bubble size = star count
            color="language",
